...
import { getAccountPortfolio } from "../../../redux/slices/Burrow/accountSelectors";
import { getAssets, getTotalSupplyAndBorrowUSD } from "../../../redux/slices/Burrow/assetsSelectors";
import { computePoolsDailyAmount } from "../../../redux/slices/Burrow/Selectors/getAccountRewards";
import { getStaking } from "../../../redux/slices/Burrow/Selectors/getStaking";
import { getConfig } from "../../../redux/slices/Burrow/appSelectors";
import { useSelector } from "react-redux";
import { shrinkToken } from "../../../store";
import { getNetTvlAPY, getTotalNetTvlAPY } from "../../../redux/slices/Burrow/Selectors/getNetAPY";
import { useNonFarmedAssets } from "./useNonFarmedAssets";
...
export function useExtraAPY({ tokenId: assetId, isBorrow }) {
  const [totalSupplyUSD, totalBorrowUSD] = useSelector(getTotalSupplyAndBorrowUSD(assetId));
  const { xBRRR, extraXBRRRAmount } = useSelector(getStaking);
  const portfolio = useSelector(getAccountPortfolio);
  const appConfig = useSelector(getConfig);
  const assets = useSelector(getAssets);
  const userNetTvlAPY = useSelector(getNetTvlAPY({ isStaking: false }));
  const totalNetTvlApy = useSelector(getTotalNetTvlAPY);
