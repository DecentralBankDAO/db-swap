...
import { fetchAssets, fetchRefPrices } from '../../redux/slices/Burrow/assetsSlice';
...
const SwapContainerWrapper = ({ accountId, isLoading, geoInfo}) => {
    ...
    return (
        ...
        {isLoading 
            ...
            ? <BlockedCountry/> 
            : <>
                {!transactionHash && <Tabs tab={burrowTab} setTab={setBurrowTab} tabsList={burrowTabsList} tabs={burrowTabs}/>}
                <AccountInfo/>
                {burrowTab === burrowTabs.MINT 
                    ? <>
                        {!transactionHash && <Tabs tab={tab} setTab={setTab} tabsList={tabsList} tabs={tabs}/>}
                        {tab === 'USDT' 
